...
#if defined(NDEBUG)
# error "Cgxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x62f0234cab0dbfa058f8f96fc592c70a4df521bda3265841eb2e3d3e6c17732b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cgxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cgxcoin Signed Message:\n";
...
{
    // Cgxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // Cgxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 15 * 24 * 60 * 60; // Cgxcoin: 15 days
static const int64 nTargetSpacing = 1 * 60; // Cgxcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cgxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cgxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x62f0234cab0dbfa058f8f96fc592c70a4df521bda3265841eb2e3d3e6c17732b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 30th March 2018, Facebook 'ugly truth' growth memo haunts firm";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1522681891;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1737128;

        if (fTestNet)
        {
            block.nTime    = 1522414428;
            block.nNonce   = 1737128;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x74e51ff4e258cede01107a44da032e688de8ec0eaee0e24b787d0dd564826bfb"));
        //Rather than manually generating the Genesis Block,
        //this custom code will make the genesis block for the
        //first time. This code is actually extracted from GenesisH0
        //where it uses python to generate genesisblock following
        //the given nTime (timestamp) and nOnce.
        //This set of code gives the ability to add genesis block automatically
        //============= REGION: Automated Genesis Block Generation =============//
        if (true && block.GetHash() != hashGenesisBlock)
        {
            ...
            //=========== END-REGION: Automated Genesis Block Generation ===========//
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xdb }; // Cgxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CgxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CgxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CgxcoinMiner(CWallet *pwallet)
{
    printf("CgxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cgxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CgxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CgxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CgxcoinMiner, pwallet));
}
